import { fetchRegisterAdminService } from 'api/products';
import { makeAutoObservable } from 'mobx';

export class RegisterAdminApiStore {
    isLoading = false;
    isError = false;
    statusCode = null;
    ...
    async handleRegisterAdminService(body) {
        try {
            this.updateIsError(false);
            this.updateStatusCode(null);
            this.updateIsLoading(true);
            const result = await fetchRegisterAdminService(body);
            if (result.error) {
